...
class MainFrame(wx.Frame):
    ...
    def on_load_data(self, event):
        try:
            ...
            if not os.path.exists('doctor11.json') or not os.path.exists('patin11.json'):
                wx.MessageBox("Không tìm thấy file doctor1.json hoặc patin1.json", 
                             "Lỗi", wx.OK | wx.ICON_ERROR)
                return
            
            with open('doctor11.json', 'r', encoding='utf-8') as f:
                self.doctors = json.load(f)
            with open('patin11.json', 'r', encoding='utf-8') as f:
